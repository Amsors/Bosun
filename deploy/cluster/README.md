...
```bash
kubectl label node node-hk-dogyun-1c1g \
...
最终 runner 必须同时具有 `self-hosted`、`linux`、`bosun-deploy` 标签，与 `.github/workflows/deploy.yml` 的 `runs-on` 完全匹配。T0.7 首次部署烟测阶段首选在独立低权限账号下以前台 `./run.sh` 临时运行，暂不安装 systemd service；终端关闭或主机休眠后 runner 离线属于预期状态。进入持续生产部署阶段前必须重新评审常驻 runner 的主机与服务管理方案，不得默认沿用烟测期的临时可用性。
...
- Repository Variable：`ACR_REGISTRY`，只填 registry endpoint，不含 `/amsors`；
- Repository Variable：`ACME_EMAIL`，填 Let's Encrypt 账号与证书到期通知邮箱；
- `registry` Environment Secrets：`ACR_USERNAME`、`ACR_PASSWORD`、`DOCKERHUB_USERNAME`、`DOCKERHUB_TOKEN`；
...
发布与部署均为显式人工操作：先确认目标 commit 的 `CI / ci-gate` 绿色，再手动运行 `Publish images` 并输入完整 40 位 commit SHA；该工作流全量构建六个统一七位 SHA tag 的镜像并同时推送 ACR 与 Docker Hub。镜像全部发布成功后，手动运行 `Deploy production` 并输入相同完整 SHA。工作流不再因 main 提交或镜像发布成功自动部署生产。任何缺失的 Variable 或 Secret 都会在使用前失败，检查过程只报告配置项名称，不输出其值。
